    ...
    row.update(
        {
            ...
            "profit_units": None,
            # Explicit zero stake prevents the virtual-bank backfiller from
            # inventing money for a Brain signal that had no required price.
...
def _menu_reconcile() -> int:
    ...
    for row in rows:
        ...
        halftime = str(state.get("status_code") or "") == "38"
        ...
        if halftime and family == "first_half_total":
            continue
    return changed
