...
class LiveSnapshotCollector:
    ...
    def __init__(self, data_dir: str | Path = "data/raw/live", prefilter_threshold: float = 50.0, secondary_interval_minutes: int = 3) -> None:
        self.data_dir = Path(data_dir); self.prefilter_threshold = float(prefilter_threshold); self.secondary_interval_minutes = max(1, int(secondary_interval_minutes))
        self.flashscore = FlashscoreProvider(); self.fotmob = FotMobProvider(); self.scores365 = Scores365Provider()
        self._last_secondary_minute: dict[str, int] = {}; self._tracked_matches: dict[str, dict[str, Any]] = {}; self._prematch_context: dict[str, dict[str, Any]] = {}; self._stop = False

    @staticmethod
    def _eligible_for_detail(minute: int, is_halftime: bool) -> bool: return bool(is_halftime or 1 <= minute <= 90)
    ...
    def _secondary_due(self, match_id: str, minute: int) -> bool:
        ...
        if previous is None or minute - previous >= self.secondary_interval_minutes:
            self._last_secondary_minute[match_id] = minute; return True
        return False

    def _path_for_now(self, now: datetime) -> Path: return self.data_dir / f"{now.date().isoformat()}.jsonl"
    ...
    def _append(self, record: dict[str, Any], now: datetime) -> None:
        ...
        with path.open("a", encoding="utf-8") as handle: handle.write(json.dumps(record, ensure_ascii=False, separators=(",", ":")) + "\n")
    ...
    def _attach_flashscore_assets(self, record: dict[str, Any], match_meta: dict[str, Any]) -> None:
        ...
        home_logo = self.flashscore.team_logo_url(str(match_meta.get("home_team_slug") or ""), str(match_meta.get("home_team_id") or "")); away_logo = self.flashscore.team_logo_url(str(match_meta.get("away_team_slug") or ""), str(match_meta.get("away_team_id") or ""))
    ...
    def _history_for(self, match: Any) -> dict[str, Any]:
        ...
        if mid not in self._prematch_context:
            try:
                ...
                print(f"prematch_history_error match={mid} error={type(exc).__name__}:{exc}", flush=True); self._prematch_context[mid] = {"source": "flashscore_h2h", "error": str(exc)}
    ...
    def collect_once(self) -> dict[str, int]:
        ...
        for match in matches:
            try:
                mid = str(match.provider_match_id); self._tracked_matches[mid] = {"home": match.home, "away": match.away, "league": match.league, "meta": dict(match.meta or {})}; minute = int(match.minute or 0)
        ...
        if missing:
            try:
                ...
                for mid in list(missing):
                    ...
                    self._tracked_matches.pop(mid, None); self._last_secondary_minute.pop(mid, None); self._prematch_context.pop(mid, None)
            except Exception as exc:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Collect GOOL live football snapshots"); parser.add_argument("--data-dir", default=os.getenv("RUNTIME_DATA_DIR", "data") + "/raw/live"); parser.add_argument("--interval", type=int, default=int(os.getenv("LIVE_INTERVAL_SECONDS", "60"))); parser.add_argument("--prefilter", type=float, default=float(os.getenv("CORE_ANALYSIS_PREFILTER", "50"))); parser.add_argument("--secondary-interval", type=int, default=int(os.getenv("SECONDARY_PROVIDER_INTERVAL_MINUTES", "3")))
    args = parser.parse_args(); collector = LiveSnapshotCollector(data_dir=args.data_dir, prefilter_threshold=args.prefilter, secondary_interval_minutes=args.secondary_interval); signal.signal(signal.SIGINT, collector.stop); signal.signal(signal.SIGTERM, collector.stop); collector.run_forever(interval_seconds=args.interval)
