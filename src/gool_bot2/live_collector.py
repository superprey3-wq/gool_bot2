...
class LiveSnapshotCollector:
    ...
    def _history_for(self, match: Any) -> dict[str, Any]:
        ...
        if mid not in self._prematch_context:
            try:
                self._prematch_context[mid] = self.flashscore.fetch_match_history(mid, str(match.home), str(match.away), limit=int(os.getenv("PREMATCH_HISTORY_MATCHES", "10")))
            except Exception as exc:
                print(f"prematch_history_error match={mid} error={type(exc).__name__}:{exc}", flush=True)
                self._prematch_context[mid] = {"source": "flashscore_h2h", "error": str(exc)}
    ...
    def _refresh_secondary(self, match: Any, minute: int) -> int:
        """Refresh FotMob/365Scores independently of the Flashscore prefilter.

        Their last successful snapshots stay attached on every minute between
        refreshes, so the signal worker sees a stable 1-3 provider context instead
        of receiving secondary data only on occasional candidate minutes.
        """
        mid = str(match.provider_match_id)
        cache = self._secondary_cache.setdefault(mid, {})
        refreshed = 0
        if not self._secondary_due(mid, minute):
            return refreshed
        try:
            fm = self.fotmob.enrich(match.home, match.away)
            if fm:
                cache[fm.provider] = self._provider_payload(fm); refreshed += 1
        except Exception as exc:
            print(f"fotmob_enrich_error match={mid} error={type(exc).__name__}:{exc}", flush=True)
        try:
            sc = self.scores365.enrich(match.home, match.away)
            if sc:
                cache[sc.provider] = self._provider_payload(sc); refreshed += 1
        except Exception as exc:
            print(f"scores365_enrich_error match={mid} error={type(exc).__name__}:{exc}", flush=True)
        return refreshed
    ...
    def _live_record(self, match: Any, now: datetime) -> dict[str, Any]:
        ...
        record: dict[str, Any] = {
            ...
            "match": {"flashscore_event_id": match.provider_match_id, "home": match.home, "away": match.away, "league": match.league, "minute": minute, "home_score": match.home_score, "away_score": match.away_score, "is_halftime": match.is_halftime, "status_code": match.meta.get("status_code", ""), "is_finished": False},
            "providers": {"flashscore": {"id": match.provider_match_id, "stats": {}, "meta": match.meta}},
            "prefilter": {"score": 0.0, "candidate": False, "reasons": []},
            "prematch_context": self._history_for(match),
        }
        ...
        self._attach_secondary_cache(record, str(match.provider_match_id))
        return record

    def _final_record(self, match_id: str, info: dict[str, Any], state: dict[str, Any], now: datetime) -> dict[str, Any]:
        goals = self.flashscore.fetch_goal_timeline(match_id); meta = dict(info.get("meta") or {})
        ...
        self._attach_secondary_cache(record, match_id)
        return record

    def collect_once(self) -> dict[str, int]:
        now = datetime.now(timezone.utc); matches = self.flashscore.live_matches(); current_ids = {str(m.provider_match_id) for m in matches}
        counters = {"live":len(matches),"detail":0,"candidate":0,"secondary":0,"final":0,"errors":0}
        for match in matches:
            try:
                mid = str(match.provider_match_id); minute = int(match.minute or 0)
                ...
                counters["secondary"] += self._refresh_secondary(match, minute)
        ...
        if missing:
            try:
                ...
                for mid in list(missing):
                    ...
                    self._append(final_record, now); self._append({**final_record,"captured_at":datetime.now(timezone.utc).isoformat()}, now); counters["final"] += 1
                ...
                counters["errors"] += 1; print(f"final_state_error={type(exc).__name__}:{exc}", flush=True)
        return counters
    ...
    def run_forever(self, interval_seconds: int = 60) -> None:
        ...
        while not self._stop:
            ...
            try:
                counters = self.collect_once(); print(json.dumps({"collector":counters,"at":datetime.now(timezone.utc).isoformat()}), flush=True)
            except Exception as exc:
...
def main() -> None:
    ...
    args=parser.parse_args();collector=LiveSnapshotCollector(data_dir=args.data_dir,prefilter_threshold=args.prefilter,secondary_interval_minutes=args.secondary_interval)
    signal.signal(signal.SIGINT,collector.stop);signal.signal(signal.SIGTERM,collector.stop);collector.run_forever(interval_seconds=args.interval)
