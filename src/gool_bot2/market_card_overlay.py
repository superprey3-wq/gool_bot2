...
def append_xbet_market_block(png: bytes, info: dict[str, Any] | None) -> bytes:
    ...
    state = "ПОДТВЕРЖДАЕТ" if confirmed else ("НЕТ ПРОГРУЗА" if level == "NEUTRAL" else level)
    d.text((62, y0 + 20), "1xBET MARKET PRESSURE", font=sc._font(24, True), fill=sc.TEXT)
    d.text((62, y0 + 60), state, font=sc._font(28, True), fill=sc.TEXT)
    ...
    reason_font = sc._fit(d, reason, src.width - 124, 18, True)
    d.text((62, y0 + 188), reason, font=reason_font, fill=sc.MUTED)
