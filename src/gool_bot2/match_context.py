...
def provider_pair(record: dict[str, Any], key: str, mode: str = "mean") -> tuple[float | None, float | None]:
    ...
    if not pairs:
        return None, None
    homes = [value[0] for value in pairs]; aways = [value[1] for value in pairs]
    if mode == "max": return max(homes), max(aways)
...
def card_context(record: dict[str, Any]) -> dict[str, Any]:
    yellow = provider_pair(record, "yellow_cards", mode="max"); red = provider_pair(record, "red_cards", mode="max")
    home_yellow = int(yellow[0]) if yellow[0] is not None else None; away_yellow = int(yellow[1]) if yellow[1] is not None else None
    home_red = int(red[0]) if red[0] is not None else None; away_red = int(red[1]) if red[1] is not None else None
    red_balance = None if home_red is None or away_red is None else home_red - away_red
    return {"home_yellow":home_yellow,"away_yellow":away_yellow,"home_red":home_red,"away_red":away_red,"red_balance":red_balance,"has_red_card":bool((home_red or 0)+(away_red or 0))}
...
def live_rich_features(record: dict[str, Any]) -> dict[str, float | None]:
    ...
    for provider_key,(home_key,away_key) in mapping.items():
        ...
        home,away = provider_pair(record,provider_key,mode=mode); out[home_key]=home; out[away_key]=away

    momentum = record.get("live_momentum") or {}
    aliases = {
        "home_shots_last_5m":"home_shots_last_5m","away_shots_last_5m":"away_shots_last_5m",
        "home_sot_last_5m":"home_sot_last_5m","away_sot_last_5m":"away_sot_last_5m",
        "home_xg_last_5m":"home_xg_last_5m","away_xg_last_5m":"away_xg_last_5m",
        "home_shots_last_10m":"home_shots_last_10m","away_shots_last_10m":"away_shots_last_10m",
        "home_xg_last_10m":"home_xg_last_10m","away_xg_last_10m":"away_xg_last_10m",
    }
    for feature,key in aliases.items():
        value=momentum.get(key)
        try: out[feature]=None if value is None else max(0.0,float(value))
        except (TypeError,ValueError): out[feature]=None
    return out
