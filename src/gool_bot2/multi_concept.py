...
def routing_experts(match: dict[str, Any], experts: dict[str, Any]) -> dict[str, Any]:
    ...
    The production product installs the complete journal/result/menu pipeline once
    at worker startup. These two idempotent calls remain only as a safe fallback
    for tests or direct library use that invokes routing without the product
    bootstrap. No journal, settlement or Telegram-result patch is installed here.
    """
