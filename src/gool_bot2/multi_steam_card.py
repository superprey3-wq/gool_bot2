...
from .multi_card import GOLD, PANEL2, render_multi_card
...
def steam_snapshot(market_row: dict[str, Any] | None, decision: RouterDecision) -> dict[str, Any]:
    ...
    strong = (
        pressure_pp >= STRONG_STEAM_MIN_PP
        or level in {"STRONG_STEAM", "AUTONOMOUS_STEAM"}
        or source.startswith("1xbet:autonomous_steam")
    )
    return {
        "strong": bool(strong),
...
def render_multi_signal_card(
    ...
    *,
    ...
    """Use the common information-rich card and only add strong-steam emphasis."""
    base_png = render_multi_card(record, decision, entry=entry, market_row=market_row)
    if not steam_snapshot(market_row, decision)["strong"]:
        return base_png
    ...
    draw.rounded_rectangle((45, 382, width - 45, 610), 26, outline=GOLD, width=4)
    draw.rounded_rectangle((45, 835, width - 45, 900), 20, outline=GOLD, width=3)
