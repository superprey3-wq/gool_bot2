...
def _minute(fields: dict[str, str], now: int) -> tuple[int, bool]:
    ac = fields.get("AC", "")
    ao = _as_int(fields.get("AO")); ad = _as_int(fields.get("AD"))
...
class FlashscoreProvider:
    ...
    def _parse_master_states(self, body: str) -> dict[str, dict[str, Any]]:
        ...
        for chunk in (body or "").split("~"):
            ...
            if not sep or len(event_id) != 8 or not event_id.isalnum():
                continue
            f = _fields(rest)
            coarse = str(f.get("AB") or "")
            states[event_id] = {
                "event_id": event_id,
                "coarse_status": coarse,
                "status_code": str(f.get("AC") or ""),
                "is_live": coarse == LIVE_COARSE_STATUS,
                "is_finished": coarse == FINISHED_COARSE_STATUS,
    ...
    def event_states(self, event_ids: set[str] | list[str]) -> dict[str, dict[str, Any]]:
        """Return current/finished states for tracked matches using only two master-feed requests."""
    ...
    def parse_master_live(self, body: str) -> list[ProviderMatch]:
        ...
        for chunk in (body or "").split("~"):
            ...
            meta = {
                "status_code": f.get("AC", ""), "coarse_status": f.get("AB", ""),
                "home_team_id": (f.get("JA") or "").strip(), "away_team_id": (f.get("JB") or "").strip(),
                "home_team_slug": (f.get("WU") or "").strip(), "away_team_slug": (f.get("WV") or "").strip(),
                "home_short": (f.get("WM") or "").strip(), "away_short": (f.get("WN") or "").strip(),
                "round": (f.get("ER") or "").strip(), "home_logo_file": (f.get("OA") or "").strip(), "away_logo_file": (f.get("OB") or "").strip(),
            }
            matches.append(ProviderMatch(provider=self.name, provider_match_id=event_id, home=home, away=away, minute=minute,
                home_score=_as_int(f.get("AG"), _as_int(f.get("AT"))), away_score=_as_int(f.get("AH"), _as_int(f.get("AU"))),
                league=league, is_halftime=is_ht, meta=meta))
    ...
    def fetch_goal_timeline(self, event_id: str) -> list[dict[str, Any]]:
        ...
        for chunk in (body or "").split("~III"):
            ...
            if home > last[0] or away > last[1]:
                goals.append({"minute": int(mm.group(1)), "side": "home" if home > last[0] else "away", "score": [home, away]}); last = (home, away)
        return goals
