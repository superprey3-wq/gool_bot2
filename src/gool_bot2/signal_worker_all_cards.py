...
def _pair_total(record,key):
    try:
        if key=="xg": home,away,_,_=xg_or_proxy_pair(record)
        else: home,away=provider_pair(record,key)
...
def _form_stats(rows,team=None,venue=None):
    rows=list(rows or [])
    if not rows:return {"matches":0,"goals_total":0,"goals_for":0,"goals_against":0,"avg_total":None,"avg_for":None,"avg_against":None,"scored_rate":None,"conceded_rate":None,"clean_sheet_rate":None,"under15_rate":None,"over15_rate":None,"over25_rate":None,"over35_rate":None,"zero_zero_rate":None,"one_goal_rate":None}
    totals=[];gf_values=[];ga_values=[];scored=[];conceded=[];t=(team or "").casefold().strip()
    for r in rows:
        hs=int(r.get("home_score") or 0);aws=int(r.get("away_score") or 0);totals.append(hs+aws)
        if t:
            is_home=str(r.get("home") or "").casefold().strip()==t;gf=hs if is_home else aws;ga=aws if is_home else hs;gf_values.append(gf);ga_values.append(ga);scored.append(gf>0);conceded.append(ga>0)
    n=len(rows);gt=sum(totals)
    return {"matches":n,"goals_total":gt,"goals_for":sum(gf_values),"goals_against":sum(ga_values),"avg_total":gt/n,"avg_for":sum(gf_values)/n if gf_values else None,"avg_against":sum(ga_values)/n if ga_values else None,"scored_rate":sum(scored)/n if scored else None,"conceded_rate":sum(conceded)/n if conceded else None,"clean_sheet_rate":sum(not x for x in conceded)/n if conceded else None,"under15_rate":sum(x<=1 for x in totals)/n,"over15_rate":sum(x>=2 for x in totals)/n,"over25_rate":sum(x>=3 for x in totals)/n,"over35_rate":sum(x>=4 for x in totals)/n,"zero_zero_rate":sum(x==0 for x in totals)/n,"one_goal_rate":sum(x==1 for x in totals)/n}
...
def _another_goal_live_confirmation(record):
    ...
    cumulative={"xg":None if xh is None or xa is None else float(xh+xa),"xgot":_pair_total(record,"xgot"),"shots":_pair_total(record,"shots"),"sot":_pair_total(record,"shots_on_target"),"inside":_pair_total(record,"shots_inside_box"),"big":_pair_total(record,"big_chances"),"high_xg":_pair_total(record,"high_xg_shots"),"danger":_pair_total(record,"dangerous_attacks"),"touches":_pair_total(record,"touches_box"),"corners":_pair_total(record,"corners")}
...
class CardAllMatchSignalWorker(base.AllMatchSignalWorker):
    ...
    def _process(self,record):
        ...
        if mid and _prematch_has_minimum(ctx):self._prematch_ready_cache[mid]=dict(ctx)
            ...
            record["prematch_context"]=dict(self._prematch_ready_cache[mid]);cached=record["prematch_context"];print(f"PREMATCH_CACHE_RESTORE match={mid} home={len(cached.get('home_recent') or [])} away={len(cached.get('away_recent') or [])}",flush=True)
