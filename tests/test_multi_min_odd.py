...
def test_multi_router_has_absolute_minimum_odd_1_50():
    assert ABSOLUTE_MIN_BET_ODD == 1.50

    below = score_candidate(_candidate(1.499), minute=30)
    boundary = score_candidate(_candidate(1.50), minute=30)
...
def test_value_layer_starts_at_1_50_by_default(monkeypatch):
    ...
    info = {
        ...
        "targets": [{
            ...
            "weight": 1.0,
            "label": "ТБ 0.5",
            "selection": {"odd": 1.50, "opposite": 3.50},
        }],
    }

    result = attach_value(info, 0.90, probability_source="test")

    assert result["value_min_odd"] == 1.50
    assert result["value_price_ok"] is True
    assert result["value_bet"] is True


def test_value_minimum_cannot_be_configured_below_1_50(monkeypatch):
    ...
    info = {
        ...
        "targets": [{
            ...
            "weight": 1.0,
            "label": "ТБ 0.5",
            "selection": {"odd": 1.49, "opposite": 3.50},
        }],
    }
