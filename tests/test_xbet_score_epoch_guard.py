...
def _game(score=(1, 0)):
    return {
        "SC": {"FS": {"S1": score[0], "S2": score[1]}},
        "GE": [],
    }


def _collector(tmp_path: Path, fs_score=(0, 0), xbet_score=(1, 0)):
    fs = SimpleNamespace(
        provider_match_id="m1",
        home="Home",
        away="Away",
        minute=45,
        home_score=fs_score[0],
        away_score=fs_score[1],
    )
    obj = SimpleNamespace()
    obj.flashscore = SimpleNamespace(live_matches=lambda: [fs])
